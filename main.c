...
#include "image.h"
#include "vars.h"
// Before running main.c, make sure that the array in the image.h file is updated
// with the image data and the width and height of the image
...
int main()
{
	...
	fprintf(stdout, "select an image type to convert by entering one of the following\\
	 numbers: 1 = pbm, 2 = pgm, 3 = ppm\n");
	int tmp;
	scanf_s("%d", &tmp);
		if (tmp == 1)
		{
